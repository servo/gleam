...
struct ErrorCheckingGl<T: Gl> {
    gl: T,
}

impl<T: Gl + 'static> ErrorCheckingGl<T> {
    fn maybe_wrap(fns: T) -> Rc<Gl> {
        if cfg!(feature = "assert_on_error") {
...
macro_rules! gl_apis {
    ($(fn $name:ident(&self$(, $arg:ident: $t:ty)*)$( -> $retty:ty)*;)+) => {
...
impl<T: Gl> Gl for ErrorCheckingGl<T> {
    gl_apis! {
        fn get_type(&self) -> GlType;
        fn buffer_data_untyped(&self,
                               target: GLenum,
